					...
					if ($f['n'] AND $f['r'] == $row['student_nis']) {
						...
						<tr>
							...
							<?php foreach ($bulan as $row) : ?>
								...
								?>
									<a href="<?php 
									echo ($row['bulan_status'] ==0) ?
									site_url('manage/payout/pay/' . $row['payment_payment_id'].'/'.$row['student_student_id'].'/'.$row['bulan_id']).'/?grossamount='.$row['bulan_bill'] : 
									site_url('manage/payout/not_pay/' . $row['payment_payment_id'].'/'.$row['student_student_id'].'/'.$row['bulan_id']);
									...
									if($row['bulan_status']==1) {
										...
										$stsUpdate = ['statusBayar' => 1];
										$sql = $this->db->query("SELECT * FROM transaksi_midtrans where 
												id_siswa ={$row['student_student_id']} &&
												typeBayar ='bulan' && 
												`target` ='{$row['bulan_id']}' 
												limit 1");
										...
										if($sql->num_rows() > 0)
										{
											$status_bulan = $sql->first_row();
											
											if($status_bulan->statusBayar == 0)
											{
												$cek = $this->midtrans->status($status_bulan->order_id);
												
												if($cek->transaction_status === 'settlement')
												{
													echo 'Selesai';
													
													$this->db->where('order_id', $status_bulan->order_id)
													->where("id_siswa={$row['student_student_id']}")
													->where('target', $row['bulan_id'])
													->update('transaksi_midtrans', $stsUpdate);
												}else {
													echo 'Proses';
												}
											}else{
										...
										echo number_format($row['bulan_bill'], 0, ',', '.');
									}
									?>
			...
			foreach ($bebas as $row):
				if ($f['n'] AND $f['r'] == $row['student_nis']) {
					...
					<tr class="<?php echo ($row['bebas_bill'] == $row['bebas_total_pay']) ? 'success' : 'danger' ?>">
						...
						<td>
<?php
$_sisw = $row['student_student_id'];
$_sqll = $this->db->query("SELECT statusBayar,order_id 
							FROM `transaksi_midtrans` 
							where id_siswa={$_sisw} && 
							typeBayar='bebas'
							limit 1");
$sts 	= 0;
$status = 'Belum lunas';
	...
	if($_sqll->num_rows() > 0)
	{
		...
		if($dd->statusBayar == 0)
		{
			...
			if($cek->transaction_status === 'settlement')
			{
				...
				$this->db->where("id_siswa={$row['student_student_id']}")
...
?>
							<a href="<?= site_url('manage/payout/payout_bebas/'. $row['payment_payment_id'].'/'.$row['student_student_id'].'/'.$row['bebas_id']) ?>" class="view-cicilan label <?= $classNm ?>" id="statusBayar"><?= $status ?></a>
						</td>
						...
						<div class="modal-dialog modal-md">
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<div class="row">
										<div class="col-md-6">
											...
											<input type="text" name="bebas_pay_bill" class="form-control numeric" value="<?php echo $sisa; ?>" placeholder="Jumlah Bayar" readonly>
										</div>
										<div class="col-md-6">
								...
								<div class="modal-footer">
									<button id="pay-button" data-amount ="<?php echo $sisa; ?>" type="submit" class="btn btn-success">Bayar</button>
