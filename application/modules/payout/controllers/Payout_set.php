...
// Siswa
    ...
    //$data = pdf_create($html, 'Cetak_Struk_', TRUE, 'A4', TRUE);
  }
...
// View data detail
  ...
  function pay($payment_id = NULL, $student_id =NULL, $id = NULL) {
    ...
    if ($this->input->is_ajax_request()){
		...
		$_SESSION['back-home'] = $_SERVER['HTTP_REFERER'];
		
		$this->load->view('checkout_bayar_bulan',[
			'n' => $student['period_period_id'],
			'r' => $student['student_nis'],
			'idBulan'	=> $id,
			'snapToken' => $snapToken
		]);
  ...
  function not_pay($payment_id = NULL, $student_id =NULL, $id = NULL) { 
    ...
    $this->Bulan_model->add($pay);
	$this->db->where('id_siswa', $student_id)->where('target', $id)->delete('transaksi_midtrans');
