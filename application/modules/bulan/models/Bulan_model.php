...
function add_month($data = array()) {
	...
	{
		$tmp   = [];
		$mNow  = date('m', strtotime('now'));
		$yNow  = date('Y', strtotime('now'));
	...
	public function getActiveAjaran()
	{
		$select = 'SELECT * FROM';
		$siswa  = $this->session->userdata('uid_student');
		
		$sql1 = $this->db->query("{$select} period WHERE period_status=1");
		
		if($sql1->num_rows() > 0)
		{
			$periode = $sql1->first_row();
			
			$sql2 = $this->db->query("{$select} `payment` WHERE period_period_id={$periode->period_id} limit 1");
			
			if($sql2->num_rows() > 0)
			{
				$payment = $sql2->first_row();
				
				return $payment->period_period_id;
			}
		}
